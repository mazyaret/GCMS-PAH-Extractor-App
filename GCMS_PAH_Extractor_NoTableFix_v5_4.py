#!/usr/bin/env python3
"""
GC/MS PAH Extractor - NoTableFix v5.2
...
DEFAULT_SURROGATE_SPIKE_ISU_UG_ML = 0.5
...
def _find_metadata_header(ws) -> Tuple[int, Dict[str, Optional[int]]]:
    ...
    for row in range(1, max_scan_rows + 1):
        ...
        for col in range(1, ws.max_column + 1):
            ...
            # Surrogate spike/concentration column.
            # Accept explicit labels like "Surrogate Spike (ug/mL)" and the ISU submission
            # form label "Surrogate (ug/ml)". This value is the expected surrogate
            # concentration in the final prepared extract used for recovery calculation.
            if (
                (("surrogate" in compact or "surr" in compact) and ("spike" in compact or "spiked" in compact or "target" in compact or "conc" in compact or "concentration" in compact))
                or compact in {"surrogate", "surrogate ug ml", "surrogate ug per ml", "surrogate spike", "surrogate spike conc", "surrogate spike concentration"}
                or (("surrogate" in compact or "surr" in compact) and ("ug" in compact or "microg" in compact or "µg" in compact) and "ml" in compact)
            ):
                if "ng" in compact and "ml" in compact:
                    found["surrogate_spike_ng_ml"] = col
                else:
                    found["surrogate_spike_ug_ml"] = col
    ...
    if not best_row:
        raise ValueError(
            ...
            "'Fabric size (cm^2)', 'fabric mass (g)', 'fuel mass loss (g)', and optional 'Surrogate Spike (ug/mL)'."
        )
    ...
    for row in range(header_row + 1, ws.max_row + 1):
        ...
        rec = {
            ...
            "surrogate_spike_ug_ml": (
                to_float_or_none(ws.cell(row, cols["surrogate_spike_ug_ml"]).value) if cols.get("surrogate_spike_ug_ml") else
                ((to_float_or_none(ws.cell(row, cols["surrogate_spike_ng_ml"]).value) / 1000.0) if cols.get("surrogate_spike_ng_ml") and to_float_or_none(ws.cell(row, cols["surrogate_spike_ng_ml"]).value) is not None else None)
            ),
...
def extract_gcms(input_xlsx: Path, sample_metadata=None, keep_ignored=False):
    ...
    for row_idx in range(subheader_row + 1, ws.max_row + 1):
        ...
        fabric_mass_g = None
        fuel_mass_loss_g = None
        surrogate_spike_ug_ml = DEFAULT_SURROGATE_SPIKE_ISU_UG_ML
        surrogate_spike_source = "Default ISU 0.5 µg/mL"
        ...
        if meta_rec:
            ...
            if meta_rec.get("surrogate_spike_ug_ml") is not None:
                surrogate_spike_ug_ml = meta_rec.get("surrogate_spike_ug_ml")
                surrogate_spike_source = "Manual/submission form override"
        ...
        record = {
            ...
            "surrogate_spike_ug_ml": float(surrogate_spike_ug_ml) if surrogate_spike_ug_ml not in (None, "") else DEFAULT_SURROGATE_SPIKE_ISU_UG_ML,
            "surrogate_spike_source": surrogate_spike_source,
    ...
    meta = {
        ...
        "surrogate_recovery_formula": "surrogate_recovery_pct = surrogate_corrected_conc / sample_surrogate_spike_ug_ml * 100",
    }
    return samples, meta
...
def surrogate_recovery_pct_isu(sample: Dict[str, Any], surrogate: str) -> Optional[float]:
    ...
    spike = sample.get("surrogate_spike_ug_ml") or DEFAULT_SURROGATE_SPIKE_ISU_UG_ML
...
def write_processed_workbook(samples, meta, output_path: Path) -> None:
    ...
    ws_info.append(["GC/MS PAH Extraction Output v5.4 - ISU format"])
    ws_info.append(["Corrected concentration = Raw Final Conc. × Dilution Factor"])
    ws_info.append(["Surrogate recovery % = Surrogate Corrected Conc. ÷ sample-level Surrogate Spike Conc. × 100"])
    ws_info.append(["Default ISU surrogate spike = 0.5 µg/mL for 10 µL of 50 µg/mL into 1 mL; use 2.0 µg/mL for 40 µL of 50 µg/mL into 1 mL."])
    ...
    ws_samples.append(["Sample Name", "Data File", "Acquisition Date/Time", "Dilution Factor", "DF Source", "Extract Volume (mL)", "Fabric Size (cm^2)", "Fabric Mass (g)", "Fuel Mass Loss (g)", "Surrogate Spike (ug/mL)", "Surrogate Spike Source", "Parameter Source", "Original GC/MS Row"])
    for s in samples:
        ws_samples.append([s["sample_name"], s["data_file"], s["acq_datetime"], s["dilution_factor"], s["df_source"], s["extract_volume_ml"], s["fabric_size_cm2"], s["fabric_mass_g"], s["fuel_mass_loss_g"], s.get("surrogate_spike_ug_ml"), s.get("surrogate_spike_source"), s["parameter_source"], s["row"]])

    wide_headers = ["Sample Name", "Data File", "Acquisition Date/Time", "Dilution Factor", "DF Source", "Extract Volume (mL)", "Fabric Size (cm^2)", "Fabric Mass (g)", "Fuel Mass Loss (g)", "Surrogate Spike (ug/mL)", "Surrogate Spike Source"]
    ...
    for s in samples:
        row = [s["sample_name"], s["data_file"], s["acq_datetime"], s["dilution_factor"], s["df_source"], s["extract_volume_ml"], s["fabric_size_cm2"], s["fabric_mass_g"], s["fuel_mass_loss_g"], s.get("surrogate_spike_ug_ml"), s.get("surrogate_spike_source")]
        for analyte, role, spike in ANALYTES:
            ...
            if role == "Surrogate":
                sample_spike = s.get("surrogate_spike_ug_ml") or DEFAULT_SURROGATE_SPIKE_ISU_UG_ML
                recovery = corrected / sample_spike * 100 if isinstance(corrected, (int, float)) and sample_spike else None
                row.append(recovery)
        for label in ["Total PAHs", "LMW PAHs", "HMW PAHs"]:
    ...
    for s in samples:
        for analyte, role, spike in ANALYTES:
            ...
            sample_spike = s.get("surrogate_spike_ug_ml") or DEFAULT_SURROGATE_SPIKE_ISU_UG_ML
            recovery = corrected / sample_spike * 100 if role == "Surrogate" and isinstance(corrected, (int, float)) and sample_spike else None
            ws_long.append([s["sample_name"], s["data_file"], s["acq_datetime"], s["dilution_factor"], s["extract_volume_ml"], s["fabric_size_cm2"], s["fabric_mass_g"], s["fuel_mass_loss_g"], analyte, role, raw, corrected, norm.get("mass_ug"), norm.get("surface_load"), norm.get("ug_g_fabric"), norm.get("ug_g_fuel_loss"), assigned, rec_pct, rec_factor, rec_corr, rec_norm.get("mass_ug"), rec_norm.get("surface_load"), rec_norm.get("ug_g_fabric"), rec_norm.get("ug_g_fuel_loss"), tef, teq_corr, teq_norm.get("surface_load"), teq_norm.get("ug_g_fabric"), teq_norm.get("ug_g_fuel_loss"), teq_rec_corr, teq_rec_norm.get("surface_load"), teq_rec_norm.get("ug_g_fabric"), teq_rec_norm.get("ug_g_fuel_loss"), sample_spike, recovery])
    ...
    for s in samples:
        for analyte, role, spike in ANALYTES:
            ...
            ws_qc.append([s["sample_name"], analyte, corrected, sample_spike, recovery, flag])

    ws_config.append(["Analyte", "Role", "Group", "Default Surrogate Spike Conc. (ug/mL)", "TEF", "Assigned Recovery Surrogate"])
    for analyte, role, spike in ANALYTES:
    ...
    ws_run.append(["ISU surrogate spike note", "Default is 0.5 µg/mL; enter 2.0 µg/mL for 40 µL of 50 µg/mL surrogate in 1 mL final solution, or any sample-specific value."])
...
def main():
    parser = argparse.ArgumentParser(description="Extract GC/MS PAH Final Conc. values, apply dilution factors, and compute PAH normalization and sample-level surrogate recovery metrics.")
