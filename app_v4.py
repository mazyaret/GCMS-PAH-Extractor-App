...
from GCMS_PAH_Extractor_NoTableFix_v3 import (
    extract_gcms,
    load_sample_metadata,
    write_processed_workbook,
)
from GCMS_PAH_Extractor_UIowa_v1 import (
    extract_uiowa,
...
st.write(
    ...
    "normalization inputs, and download one processed Excel workbook."
)
        ...
        if st.button("Process ISU-format GC/MS file", type="primary", key="isu_process"):
            with st.spinner("Processing ISU-format file..."):
                with tempfile.TemporaryDirectory() as tmp:
                    ...
                    output_name = Path(gcms_file.name).stem + "_processed_v4_ISU.xlsx"
...
def run_uiowa_tab():
    ...
    try:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for s in samples:
                ...
                rows.append({
                    ...
                    "Fabric Mass (g)": value_or_blank(s.get("fabric_mass_g")),
                    "Fuel Mass Loss (g)": value_or_blank(s.get("fuel_mass_loss_g")),
                    "Surrogate Spike (ng/mL)": float(s.get("surrogate_spike_ng_ml") or 500.0),
                    "DF Source": s.get("df_source"),
        ...
        st.markdown("### Review or edit dilution and normalization inputs")
        st.write(
            "For UIowa output, Calc. Conc. is treated as ng/mL. The normalized outputs use µg units "
            "after converting ng/mL to µg/mL."
        )
        edited_df = st.data_editor(
            metadata_table,
            use_container_width=True,
            ...
            column_config={
                ...
                "Fabric Mass (g)": st.column_config.NumberColumn("Fabric Mass (g)", min_value=0.0, step=0.01, format="%.6f"),
                "Fuel Mass Loss (g)": st.column_config.NumberColumn("Fuel Mass Loss (g)", min_value=0.0, step=0.01, format="%.6f"),
                "Surrogate Spike (ng/mL)": st.column_config.NumberColumn("Surrogate Spike (ng/mL)", min_value=0.0, step=50.0, format="%.4f"),
            },
        ...
        if st.button("Process University of Iowa files", type="primary", key="uiowa_process"):
            with st.spinner("Processing University of Iowa files..."):
                with tempfile.TemporaryDirectory() as tmp:
                    ...
                    output_name = Path(target_file.name).stem + "_processed_v4_UIowa.xlsx"
                    output_path = tmp / output_name
...
st.divider()
st.caption("GC/MS PAH Extractor v4 — unified platform for ISU-format and University of Iowa-format GC/MS outputs.")
