#!/usr/bin/env python3
"""
GC/MS PAH Extractor - University of Iowa format v2
    ...
    for r in range(1, min(ws.max_row, 100) + 1):
        ...
        for c in range(1, ws.max_column + 1):
            ...
            if "fuel" in label and "mass" in label and "loss" in label:
                ...
                score += 1
            if "surrogate" in label and "spike" in label:
                if "ng" in label:
...
def load_submission_metadata(submission_xlsx: Optional[Path]) -> Dict[str, Dict[str, Any]]:
    ...
    for r in range(header_row + 1, ws.max_row + 1):
        ...
        # Default surrogate spike to 0.5 µg/mL if missing, but keep source visible.
        if rec["surrogate_spike_ug_ml"] is None:
            rec["surrogate_spike_ug_ml"] = 0.5
...
def metadata_from_rows(rows: List[Dict[str, Any]]) -> Dict[str, Dict[str, Any]]:
    ...
    for idx, row in enumerate(rows):
        ...
        rec = {
            ...
            "surrogate_spike_ug_ml": positive_or_none(row.get("Surrogate Spike (µg/mL)")) or 0.5,
...
def extract_uiowa(target_xlsx: Path, surrogate_xlsx: Path, sample_metadata: Optional[Dict[str, Dict[str, Any]]] = None):
    ...
    for nm in all_sample_names:
        ...
        rec = {
            ...
            "surrogate_spike_ug_ml": positive_or_none(md.get("surrogate_spike_ug_ml")) or 0.5,
...
def write_processed_workbook_uiowa(samples: List[Dict[str, Any]], meta: Dict[str, Any], output_path: Path) -> None:
    ...
    ws_info.append(["Mass (ug) = Corrected Conc. (µg/mL) × Extract Volume (mL)"])
    ...
    for s in samples:
        ...
        for analyte in TARGET_16_PAHS:
            ...
            corrected = raw * s["dilution_factor"] / 1000.0 if isinstance(raw, (int, float)) else None
        ...
        for surr in SURROGATES_UIOWA:
            ...
            corrected = raw * s["dilution_factor"] / 1000.0 if isinstance(raw, (int, float)) else None
            spike = s.get("surrogate_spike_ug_ml") or 0.5
    ...
    ws_long.append([
        ...
        "Raw Calc. Conc. (ng/mL)", "Corrected Conc. (µg/mL)", "Mass (ug)",
        "surface load (ug/100 cm^2)", "ug/g fabric", "ug/g mass loss of fuel", "TEF", "TEQ Corrected Conc. (µg/mL)",
    ])
    for s in samples:
        for analyte in TARGET_16_PAHS:
            raw = s["target_values"].get(analyte)
            corr = raw * s["dilution_factor"] / 1000.0 if isinstance(raw, (int, float)) else None
