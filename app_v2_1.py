...
from GCMS_PAH_Extractor_NoTableFix_v2_1 import extract_gcms, load_df_overrides

st.set_page_config(page_title="GC/MS PAH Extractor", page_icon="🧪", layout="centered")

st.title("GC/MS PAH Extraction Platform v2.1")
...
def build_df_table(gcms_upload, df_upload):
    with tempfile.TemporaryDirectory() as tmp:
        ...
        samples, meta = extract_gcms(gcms_path, df_overrides=overrides)
        rows = []
        match_count = 0
        for s in samples:
            ...
            source = s["df_source"]
            if source == "Manual/submission form override":
        ...
        if st.button("Process GC/MS file", type="primary"):
            with st.spinner("Processing file..."):
                with tempfile.TemporaryDirectory() as tmp:
                    ...
                    script_path = Path(__file__).parent / "GCMS_PAH_Extractor_NoTableFix_v2_1.py"
...
st.divider()
st.caption("GC/MS PAH Extractor v2.1 — upload, review DF, process, download. DF matching handles sample labels with or without 8X.")
