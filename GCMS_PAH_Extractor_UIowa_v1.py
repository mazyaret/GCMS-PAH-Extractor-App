#!/usr/bin/env python3
"""
GC/MS PAH Extractor - University of Iowa format v1
...
- UIowa Calc. Conc. values are treated as ng/mL because the calibration files and submission
  form use ng/mL. Normalization outputs in µg units convert ng/mL to µg/mL by dividing by 1000.
...
def to_float_or_none(value: Any) -> Optional[float]:
    ...
    if isinstance(value, str):
        ...
        if not s:
            return None
        # handle strings like "1mL", "112.5 cm2", "8X"
    ...
    for r in range(1, min(ws.max_row, 100) + 1):
        ...
        for c in range(1, ws.max_column + 1):
            ...
            if "fuel" in label and "mass" in label and "loss" in label:
                ...
                score += 1
            if "surrogate" in label and ("ng" in label or "spike" in label):
...
def load_submission_metadata(submission_xlsx: Optional[Path]) -> Dict[str, Dict[str, Any]]:
    ...
    for r in range(header_row + 1, ws.max_row + 1):
        ...
        rec = {
            ...
            "surrogate_spike_ng_ml": positive_or_none(ws.cell(r, cols["surrogate_spike_ng_ml"]).value) if cols.get("surrogate_spike_ng_ml") else None,
        ...
        # Default surrogate spike to 500 ng/mL if missing, but keep source visible.
        if rec["surrogate_spike_ng_ml"] is None:
            rec["surrogate_spike_ng_ml"] = 500.0
...
def metadata_from_rows(rows: List[Dict[str, Any]]) -> Dict[str, Dict[str, Any]]:
    ...
    for idx, row in enumerate(rows):
        ...
        rec = {
            ...
            "surrogate_spike_ng_ml": positive_or_none(row.get("Surrogate Spike (ng/mL)")) or 500.0,
...
def extract_uiowa(target_xlsx: Path, surrogate_xlsx: Path, sample_metadata: Optional[Dict[str, Dict[str, Any]]] = None):
    ...
    for nm in all_sample_names:
        ...
        rec = {
            ...
            "surrogate_spike_ng_ml": positive_or_none(md.get("surrogate_spike_ng_ml")) or 500.0,
    ...
    meta = {
        ...
        "assumed_calc_conc_unit": "ng/mL",
        "normalization_note": "Mass in ug = Corrected Conc. (ng/mL) / 1000 * Extract Volume (mL)",
    }
    return samples, meta


def calc_norms(corrected_ng_ml: Optional[float], sample: Dict[str, Any]) -> Dict[str, Optional[float]]:
    if not isinstance(corrected_ng_ml, (int, float)):
        return {"mass_ug": None, "surface_load": None, "ug_g_fabric": None, "ug_g_fuel_loss": None}
    mass_ug = corrected_ng_ml / 1000.0 * (sample.get("extract_volume_ml") or 1.0)
...
def _summary_for_group(sample: Dict[str, Any], analytes: List[str], teq: bool = False) -> Dict[str, Optional[float]]:
    ...
    for a in analytes:
        ...
        if not isinstance(raw, (int, float)):
            continue
        corr = raw * sample["dilution_factor"]
    ...
    return {
        "corrected_ng_ml": _sum_values(corrected_vals),
...
def write_processed_workbook_uiowa(samples: List[Dict[str, Any]], meta: Dict[str, Any], output_path: Path) -> None:
    ...
    ws_info.append(["University of Iowa GC/MS PAH Extraction Output"])
    ws_info.append(["Raw Calc. Conc. values are treated as ng/mL."])
    ws_info.append(["Corrected Conc. (ng/mL) = Raw Calc. Conc. (ng/mL) × Dilution Factor"])
    ws_info.append(["Mass (ug) = Corrected Conc. (ng/mL) / 1000 × Extract Volume (mL)"])
    ...
    ws_info.append(["Surrogate recovery % = Corrected surrogate concentration (ng/mL) ÷ Surrogate Spike (ng/mL) × 100"])
    ...
    ws_samples.append([
        ...
        "Surrogate Spike (ng/mL)", "Parameter Source",
    ])
    for s in samples:
        ws_samples.append([
            ...
            s["surrogate_spike_ng_ml"], s["parameter_source"],
        ])
    ...
    for analyte in TARGET_16_PAHS:
        wide_headers.extend([
            ...
            f"{analyte} Corrected Conc. (ng/mL)",
    ...
    for label in ["Total PAHs", "LMW PAHs", "HMW PAHs"]:
        wide_headers.extend([
            f"{label} Corrected Conc. (ng/mL)",
            f"{label} surface load (ug/100 cm^2)",
            f"{label} ug/g fabric",
            f"{label} ug/g mass loss of fuel",
        ])
    ...
    for surr in SURROGATES_UIOWA:
        wide_headers.extend([
            f"{surr} Raw Calc. Conc. (ng/mL)",
            f"{surr} Corrected Conc. (ng/mL)",
            f"{surr} Recovery %",
        ])
    ...
    for s in samples:
        ...
        for analyte in TARGET_16_PAHS:
            ...
            row.extend([raw, corrected, norms["surface_load"], norms["ug_g_fabric"], norms["ug_g_fuel_loss"]])

        for analytes in [TARGET_16_PAHS, LMW_PAHS, HMW_PAHS]:
            summ = _summary_for_group(s, analytes, teq=False)
            row.extend([summ["corrected_ng_ml"], summ["surface_load"], summ["ug_g_fabric"], summ["ug_g_fuel_loss"]])
        for analytes in [TARGET_16_PAHS, LMW_PAHS, HMW_PAHS]:
        ...
        for surr in SURROGATES_UIOWA:
            ...
            corrected = raw * s["dilution_factor"] if isinstance(raw, (int, float)) else None
            spike = s.get("surrogate_spike_ng_ml") or 500.0
            recovery = corrected / spike * 100 if isinstance(corrected, (int, float)) and spike else None
            row.extend([raw, corrected, recovery])
        ws_wide.append(row)

    ws_long.append([
        "Sample Name", "UIowa Sample Key", "Data File", "Dilution Factor", "DF Source", "Extract Volume (mL)",
        "Fabric Size (cm^2)", "Fabric Mass (g)", "Fuel Mass Loss (g)", "Analyte", "Role",
        "Raw Calc. Conc. (ng/mL)", "Corrected Conc. (ng/mL)", "Mass (ug)",
        "surface load (ug/100 cm^2)", "ug/g fabric", "ug/g mass loss of fuel", "TEF", "TEQ Corrected Conc. (ng/mL)",
    ])
    for s in samples:
        for analyte in TARGET_16_PAHS:
            raw = s["target_values"].get(analyte)
            corr = raw * s["dilution_factor"] if isinstance(raw, (int, float)) else None
            norms = calc_norms(corr, s)
            tef = TEF_FACTORS.get(analyte)
            ws_long.append([
                s["sample_name"], s["uiowa_sample_key"], s["data_file"], s["dilution_factor"], s["df_source"], s["extract_volume_ml"],
                s["fabric_size_cm2"], s["fabric_mass_g"], s["fuel_mass_loss_g"], analyte, "Target",
                raw, corr, norms["mass_ug"], norms["surface_load"], norms["ug_g_fabric"], norms["ug_g_fuel_loss"], tef,
                corr * tef if isinstance(corr, (int, float)) else None,
            ])

    ws_qc.append(["Sample Name", "Surrogate", "Raw Calc. Conc. (ng/mL)", "Corrected Conc. (ng/mL)", "Spike Conc. (ng/mL)", "Recovery %", "QC Flag"])
    for s in samples:
        for surr in SURROGATES_UIOWA:
            raw = s["surrogate_values"].get(surr)
            corrected = raw * s["dilution_factor"] if isinstance(raw, (int, float)) else None
    ...
    ws_config.append(["Analyte", "Role", "Group", "TEF", "Notes"])
    for a in TARGET_16_PAHS:
        ws_config.append([a, "Target", "LMW" if a in LMW_PAHS else "HMW", TEF_FACTORS.get(a), "UIowa PAH target"])
    for surr in SURROGATES_UIOWA:
        ws_config.append([surr, "Surrogate", "", None, "UIowa surrogate"])
