#!/usr/bin/env python3
"""
GC/MS PAH Extractor - NoTableFix v3
...
def write_processed_workbook(samples, meta, output_path: Path) -> None:
    ...
    ws_info.append(["GC/MS PAH Extraction Output v3"])
    ws_info.append(["Corrected concentration = Raw Final Conc. × Dilution Factor"])
    ws_info.append(["Analyte mass (ug) = Corrected Conc. × Extract Volume (mL). Default Extract Volume is 1 mL if not provided."])
    ...
    ws_info.append(["LMW PAHs: Naphthalene, Acenaphthylene, Acenaphthene, Fluorene, Phenanthrene, Anthracene."])
    ws_info.append(["HMW PAHs: the remaining 10 PAHs in the 16-PAH set."])
    ...
    for analyte, role, spike in ANALYTES:
        ...
        if analyte in TARGET_16_PAHS:
            wide_headers.extend([f"{analyte} surface load (ug/100 cm^2)", f"{analyte} ug/g fabric", f"{analyte} ug/g mass loss of fuel"])
        if role == "Surrogate":
            wide_headers.append(f"{analyte} Recovery %")
    for label in ["Total PAHs", "LMW PAHs", "HMW PAHs"]:
        wide_headers.extend([f"{label} Corrected Conc.", f"{label} surface load (ug/100 cm^2)", f"{label} ug/g fabric", f"{label} ug/g mass loss of fuel"])
    for label in ["Total PAHs", "LMW PAHs", "HMW PAHs"]:
        wide_headers.extend([f"{label} TEQ Corrected Conc.", f"{label} TEQ surface load (ug/100 cm^2)", f"{label} TEQ ug/g fabric", f"{label} TEQ ug/g mass loss of fuel"])
    ...
    for s in samples:
        ...
        for analyte, role, spike in ANALYTES:
            ...
            if analyte in TARGET_16_PAHS:
                ...
                row.extend([norm.get("surface_load"), norm.get("ug_g_fabric"), norm.get("ug_g_fuel_loss")])
            if role == "Surrogate":
    ...
    ws_summary.append(["Sample Name", "Category", "Corrected Conc.", "surface load (ug/100 cm^2)", "ug/g fabric", "ug/g mass loss of fuel", "TEQ Corrected Conc.", "TEQ surface load (ug/100 cm^2)", "TEQ ug/g fabric", "TEQ ug/g mass loss of fuel"])
    for s in samples:
        for label in ["Total PAHs", "LMW PAHs", "HMW PAHs"]:
            m = s["summary_metrics"][label]
            t = s["teq_metrics"][label]
            ws_summary.append([s["sample_name"], label, m["corrected_conc"], m["surface_load"], m["ug_g_fabric"], m["ug_g_fuel_loss"], t["corrected_conc"], t["surface_load"], t["ug_g_fabric"], t["ug_g_fuel_loss"]])

    ws_long.append(["Sample Name", "Data File", "Acquisition Date/Time", "Dilution Factor", "Extract Volume (mL)", "Fabric Size (cm^2)", "Fabric Mass (g)", "Fuel Mass Loss (g)", "Analyte", "Role", "Raw Final Conc.", "Corrected Conc.", "Analyte mass (ug)", "surface load (ug/100 cm^2)", "ug/g fabric", "ug/g mass loss of fuel", "TEF", "TEQ Corrected Conc.", "TEQ surface load (ug/100 cm^2)", "TEQ ug/g fabric", "TEQ ug/g mass loss of fuel", "Surrogate Spike", "Recovery %"])
    for s in samples:
        for analyte, role, spike in ANALYTES:
            raw = s["values"].get(analyte)
            corrected = s["corrected_values"].get(analyte)
            ...
            ws_long.append([s["sample_name"], s["data_file"], s["acq_datetime"], s["dilution_factor"], s["extract_volume_ml"], s["fabric_size_cm2"], s["fabric_mass_g"], s["fuel_mass_loss_g"], analyte, role, raw, corrected, norm.get("mass_ug"), norm.get("surface_load"), norm.get("ug_g_fabric"), norm.get("ug_g_fuel_loss"), tef, teq_corr, teq_norm.get("surface_load"), teq_norm.get("ug_g_fabric"), teq_norm.get("ug_g_fuel_loss"), spike, recovery])
    ...
    ws_config.append(["Analyte", "Role", "Group", "Surrogate Spike Conc. (ug/mL)", "TEF"])
    for analyte, role, spike in ANALYTES:
        group = "LMW PAH" if analyte in LMW_PAHS else "HMW PAH" if analyte in HMW_PAHS else "Surrogate"
        ws_config.append([analyte, role, group, spike, TEF_FACTORS.get(analyte)])
