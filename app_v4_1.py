...
from GCMS_PAH_Extractor_UIowa_v2 import (
    extract_uiowa,
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for s in samples:
                ...
                rows.append({
                    ...
                    "Fabric Mass (g)": value_or_blank(s.get("fabric_mass_g")),
                    "Fuel Mass Loss (g)": value_or_blank(s.get("fuel_mass_loss_g")),
                    "Surrogate Spike (µg/mL)": float(s.get("surrogate_spike_ug_ml") or 0.5),
                    "DF Source": s.get("df_source"),
        ...
        if st.button("Process University of Iowa files", type="primary", key="uiowa_process"):
            with st.spinner("Processing University of Iowa files..."):
                with tempfile.TemporaryDirectory() as tmp:
                    ...
                    output_name = Path(target_file.name).stem + "_processed_v4_1_UIowa.xlsx"
                    output_path = tmp / output_name
...
st.divider()
st.caption("GC/MS PAH Extractor v4.1 — unified platform for ISU-format and University of Iowa-format GC/MS outputs.")
